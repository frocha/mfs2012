                         ...
                         const GValue *value, GParamSpec *pspec)
{
        ...
        case PROP_NAME:
                ...
                break;
	case PROP_NUMBER:
		gt_sample_set_number (self, g_value_get_int (value));
		break;
        default:
                G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
        }
...
gt_sample_class_init (GtSampleClass *klass)
{
        ...
        g_object_class_install_property
                (object_class, PROP_NUMBER,
                 g_param_spec_int ("number", "Your number", "The quantity",
                                      0, 10, 5,
                                      G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}
...
void
gt_sample_set_number (GtSample *self, const int number)
{
...
gt_sample_say_hello (GtSample *self)
{
	...
	const int number = self->priv->number;
        g_print ("Hola %s %d\n", name, number);
}
